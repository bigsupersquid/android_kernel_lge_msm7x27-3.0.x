...
static struct batt_rep_chg_type batt_rep;
	...
	msm_batt.battery_therm = be32_to_cpu(batt_rep.battery_therm);
...
static unsigned charge_type;
static unsigned battery_therm;
static unsigned charger_status;
static unsigned battery_level;
...
static ssize_t msm_charger_status_show(struct device* dev, struct device_attribute* attr, char* buf)
{
	charger_status = msm_batt_info.charger_status;
	return sprintf(buf,"%d\n", charger_status);
}
static DEVICE_ATTR(charger_status, S_IRUGO, msm_charger_status_show, NULL);

static ssize_t msm_batt_charge_type_show(struct device* dev, struct device_attribute* attr, char* buf)
{
	charge_type = msm_batt_info.charger_type;
	return sprintf(buf,"%d\n", charge_type);
}
static DEVICE_ATTR(charge_type, S_IRUGO, msm_batt_charge_type_show, NULL);
...
static ssize_t msm_battery_level_show(struct device* dev, struct device_attribute* attr, char* buf)
{
	battery_level = msm_batt_info.battery_level;
	return sprintf(buf,"%d\n", battery_level);
}
static DEVICE_ATTR(battery_level, S_IRUGO, msm_battery_level_show, NULL);
