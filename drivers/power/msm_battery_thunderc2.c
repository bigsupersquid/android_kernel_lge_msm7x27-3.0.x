...
static int msm_batt_get_batt_chg_status(void)
{
	...
	if (rc < 0) {
		...
		msm_batt.batt_capacity=be32_to_cpu(v1p->batt_capacity);
		msm_batt.voltage_now=be32_to_cpu(v1p->voltage_now);
		msm_batt.batt_valid_id=be32_to_cpu(v1p->batt_valid_id);
		msm_batt.batt_therm=be32_to_cpu(v1p->batt_therm);
		msm_batt.batt_temp=be32_to_cpu(v1p->batt_temp);
		msm_batt.battery_valid=be32_to_cpu(v1p->battery_valid);
		msm_batt.battery_charging=be32_to_cpu(v1p->battery_charging);
